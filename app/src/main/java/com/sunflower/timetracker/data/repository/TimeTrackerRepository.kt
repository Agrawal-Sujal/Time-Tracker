...
class TimeTrackerRepository @Inject constructor(
    private val tagDao: TagDao, private val sessionDao: TimeSessionDao
) {
    ...
    suspend fun startSession(tagId: Long): Long {
        ...
        return sessionDao.insertSession(
            TimeSessionEntity(tagId = tagId, startTime = now, latestStartTime = now)
        )
    }
    ...
    suspend fun pauseActiveSession() {
        sessionDao.getActiveSessionOnce()?.let { active ->
            if (!active.isPaused) {
                val duration =
                    active.durationMs + (System.currentTimeMillis() - active.latestStartTime)
                sessionDao.updateSession(active.copy(isPaused = true, durationMs = duration))
            }
        }
    }
        ...
        sessionDao.getActiveSessionOnce()?.let { active ->
            if (active.isPaused) {
                ...
                sessionDao.updateSession(
                    active.copy(isPaused = false, latestStartTime = now)
                )
            }
        }
    }
    ...
    suspend fun stopActiveSession() {
        sessionDao.getActiveSessionOnce()?.let { active ->
            ...
            val duration = if (active.isPaused) {
                ...
                active.durationMs + (now - active.latestStartTime)
            }
            sessionDao.updateSession(
    ...
    suspend fun updateSession(session: TimeSession) = sessionDao.updateSession(session.toEntity())
    ...
    suspend fun getActiveSessionOnce(): TimeSession? = sessionDao.getActiveSessionOnce()?.toDomain()
    ...
    fun getWeekStats(weekOf: LocalDate = LocalDate.now()): Flow<List<TagStats>> {
        val zone = ZoneId.systemDefault()
        val weekStart =
            weekOf.with(TemporalAdjusters.previousOrSame(DayOfWeek.MONDAY)).atStartOfDay(zone)
                .toInstant().toEpochMilli()
        val weekEnd = weekOf.with(TemporalAdjusters.nextOrSame(DayOfWeek.SUNDAY)).plusDays(1)
            .atStartOfDay(zone).toInstant().toEpochMilli()
        return buildTagStats(weekStart, weekEnd)
    }

    private fun buildTagStats(from: Long, to: Long): Flow<List<TagStats>> = combine(
        tagDao.getAllTags(), sessionDao.getTagDurationsForRange(from, to)
    ) { tags, durations ->
    ...
    private fun TimeSessionEntity.toDomain() = TimeSession(
        id, tagId, startTime, endTime, durationMs, isPaused, latestStartTime
    )

    private fun TimeSession.toEntity() = TimeSessionEntity(
        id, tagId, startTime, endTime, durationMs, isPaused, latestStartTime
    )
}